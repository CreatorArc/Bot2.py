...
# Photos ke link
WELCOME_PHOTO_URL = "https://t.me/shjahshsbsb/12"     # Updated Welcome Photo Link
...
def handle_admin_action(call):
    ...
    if action == "app":
        try:
            ...
            join_btn.add(types.InlineKeyboardButton("🚀 Join Private Channel", url=GROUP_INVITE_LINK))
...
# 6. Admin Approval / Rejection Trigger (UPDATED BUTTON & LINK)
@bot.callback_query_handler(func=lambda call: call.data.startswith(("app_", "rej_")))
def handle_admin_action(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "Permission Denied!", show_alert=True)
